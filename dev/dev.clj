(ns dev
  (:require
   ...
   [yada-datomic.system :refer (new-system-map new-dependency-map)]
   [yada-datomic.todo :as todo]))

(def system nil)

(defn config
  "Return a map of the static configuration used in the component constructors."
  []
  (read-config
...
(defn test-all []
  (clojure.test/run-all-tests #"yada-datomic.*test$"))
